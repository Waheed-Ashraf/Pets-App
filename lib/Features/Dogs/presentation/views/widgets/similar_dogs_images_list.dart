...
class SimilarDogsImagesList extends StatelessWidget {
  const SimilarDogsImagesList({super.key});

  @override
  Widget build(BuildContext context) {
    int crossAxisCount = (MediaQuery.of(context).size.width / 150).round();

    return BlocBuilder<SimilarDogsImagesCubit, SimilarDogsImagesState>(
      builder: (context, state) {
        if (state is SimilarDogsImagesLoaded) {
        ...
        } else if (state is SimilarDogsImagesFailuer) {
